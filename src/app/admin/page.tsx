...
export default function AdminDashboard() {
  ...
  const [monthlyFees, setMonthlyFees] = useState<any[]>([]); // Define a proper type later
  ...
  // Keep the quick actions as they are static
  const quickActions = [
      ...
      <main className="p-4 md:p-6 lg:p-8 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <Card className="md:col-span-7 border-none shadow-sm">
             ...
             <Card className="md:col-span-5 border-none shadow-sm">
                ...
                {loading ? <div className="flex items-center justify-center h-full"><Loader2 className="h-8 w-8 animate-spin" /></div> : (
                    <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                        <Pie data={leadSources} cx="50%" cy="45%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
        ...
        <div className="grid gap-6 md:grid-cols-2">
            <Card className="border-none shadow-sm">
                {/* Attendance part will be updated later as it has its own logic */}
                <CardHeader>
            ...
            <Card className="border-none shadow-sm">
                ...
                <CardContent>
                    {loading ? <div className="space-y-3">{[...Array(3)].map((_, i) => <Skeleton key={i} className="h-16 w-full" />)}</div> : (
                        ...
                        {feeAlerts.map((alert) => (
                          <div key={alert.id} className={`flex items-center justify-between p-3 rounded-lg border-l-4 ${alert.overdue ? 'border-red-500 bg-red-50/50' : 'border-amber-500 bg-amber-50/50'}`}>
                            <div className="space-y-0.5">
                              <p className="text-sm font-bold text-[#1E2A4A]">{alert.name}</p>
                              <p className="text-xs text-muted-foreground">Class {alert.class} • Due {alert.dueDate.toDate().toLocaleDateString()}</p>
                            </div>
        ...
        <div className="grid gap-6 md:grid-cols-2">
             <Card className="border-none shadow-sm">
                ...
                <CardContent>
                {loading ? <div className="space-y-4">{[...Array(3)].map((_, i) => <Skeleton key={i} className="h-16 w-full" />)}</div> : (
                    <div className="space-y-4">
                    {recentEnquiries.map((enq) => (
                      <div key={enq.id} className="flex items-center justify-between pb-4 border-b last:border-0 last:pb-0">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-[10px] text-muted-foreground mt-1">{enq.date.toDate().toLocaleDateString()}</p>
                        </div>
                      </div>
                    ))}
